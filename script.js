...
(function(){
  // Guarantee background video playback across devices
  document.querySelectorAll('.ms-vid').forEach(function(vid) {
    if (vid.hasAttribute('data-mp4') && !vid.src) {
...
/* Video backgrounds — nothing downloads until it is needed.
   Skipped entirely (poster only) on reduced motion, data saver, or a
   small screen: a 1.5MB clip behind a phone hero is a bad trade, and a
   still frame with a Tier A overlay still feels alive. */
(function(){
  ...
  if(skip)return;                      /* the <img> poster is already showing */
    ...
    es.forEach(function(e){
      ...
      if(e.isIntersecting){
        if(!v.src&&!v.dataset.wired){
          ...
          v.addEventListener('error',function(){v.remove();},{once:true});  /* poster stands alone */
        }
...
/* Scroll choreography — ONE rAF loop for the whole page, and it only
   runs while a scene is actually on screen. Everything it does is write
   two custom properties; the CSS does the rest. */
(function(){
  ...
  /* Raw scroll offset is JUMPY — a trackpad or a smooth-scrolling mouse
     delivers uneven deltas, and driving a scale/blur straight off it makes
     the whole scene stutter even at 60fps. Easing the value toward its
     target each frame is what turns it into a camera move. 0.14 is the
     sweet spot: below ~0.08 it lags behind the finger, above ~0.25 the
     smoothing stops being visible. */
  var EASE=0.14;
  function tick(){
    ...
    // keep ticking while anything is still easing, even if the user stopped
